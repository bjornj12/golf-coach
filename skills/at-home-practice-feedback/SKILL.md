---
name: at-home-practice-feedback
description: Use when the user wants a single at-home drill with built-in feedback and validation for a diagnosed fault — a quick, phone-readable drill card (fault, root cause, drill, feedback prop, budget tier, validation checkpoint, video). Triggers on "how do I know I'm doing it right", "drill with feedback", "quick drill for X", "what can I check at home before the range".
---
...
multi-drill routine, use `golf-practice-at-home`).
...
- Pull the drill from **`drill-library`** (its `at-home-no-ball` set covers most
  faults) — it is the single source of truth for drills, feel cues, and videos.
